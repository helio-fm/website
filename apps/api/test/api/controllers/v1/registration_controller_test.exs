defmodule Api.V1.RegistrationControllerTest do
  use Api.ConnCase
  ...
  describe "register user" do
    ...
    test "renders error when no email provided", %{conn: conn} do
      conn = post conn, api_signup_path(conn, :sign_up), user: %{@data | email: nil}
      assert json_response(conn, 422)["errors"]["email"] != ""
    end

    test "renders error when invalid email provided", %{conn: conn} do
      conn = post conn, api_signup_path(conn, :sign_up), user: %{@data | email: "no_at_character"}
      assert json_response(conn, 422)["errors"]["email"] != ""
    end

    test "renders error when already provided existing email", %{conn: conn} do
      conn = post conn, api_signup_path(conn, :sign_up), user: @data
      conn = post conn, api_signup_path(conn, :sign_up), user: %{@data | login: "other-login"}
    ...
    test "renders error when already provided same email in different case", %{conn: conn} do
      conn = post conn, api_signup_path(conn, :sign_up), user: @data
      conn = post conn, api_signup_path(conn, :sign_up),
    ...
    test "renders error when no password provided", %{conn: conn} do
      conn = post conn, api_signup_path(conn, :sign_up), user: %{@data | password: nil}
    ...
    test "renders error when password confirmation doesn't match password", %{conn: conn} do
      conn = post conn, api_signup_path(conn, :sign_up), user: %{@data | password_confirmation: "mismatch"}
